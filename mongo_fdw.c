...
void DumpJson(StringInfo buffer, BSON_ITERATOR *bsonData, bool isArray);
...
MongoBeginForeignScan(ForeignScanState *scanState, int executorFlags)
{
     ...
     */
#ifdef META_DRIVER
    mongoConnection = mongo_get_connection(addressName, portNumber, databaseName, username, password, mongoFdwOptions->readPreference);
#else
...
ColumnValue(BSON_ITERATOR *bsonIterator, Oid columnTypeId, int32 columnTypeMod)
{
    ...
    switch(columnTypeId)
    {
        ...
        case JSONOID:
        {
            ...
            {
                ereport(ERROR, (errmsg("cannot convert scalar to json")));
            }
    ...
    while (BSON_ITER_NEXT(&i))
    {
        ...
        switch (t)
        {
            case BSON_TYPE_DOUBLE:
                appendStringInfo(output, "%f", BSON_ITER_DOUBLE(&i));
                break;
            case BSON_TYPE_UTF8:
                appendStringInfo(output, "\"%s\"",
                        EscapeJsonString(BSON_ITER_UTF8(&i)));
                break;
            case BSON_TYPE_SYMBOL:
                ereport(ERROR, (errcode(ERRCODE_FDW_INVALID_DATA_TYPE),
                                errmsg("`symbol` BSON type is deprecated and "
                                       "unsupported"),
                                errhint("Symbol: %s", BSON_ITER_UTF8(&i))));
                break;
            case BSON_TYPE_OID:
            {
                char oidhex[25];
            ...
            case BSON_TYPE_BOOL:
                appendStringInfoString(
                        output, BSON_ITER_BOOL(&i) ? "true" : "false");
                break;
            case BSON_TYPE_DATE_TIME:
                appendStringInfo(output, "{\"$date\":%ld}",
                        (long int)BSON_ITER_DATE_TIME(&i));
                break;
            ...
            case BSON_TYPE_REGEX:
                ereport(ERROR, (errcode(ERRCODE_FDW_INVALID_DATA_TYPE),
                                errmsg("support for `regex` BSON type is "
                                       "not implemented"),
                                errhint("Regex: %s", BSON_ITER_REGEX(&i))));
                break;
            case BSON_TYPE_CODE:
                ereport(ERROR, (errcode(ERRCODE_FDW_INVALID_DATA_TYPE),
                                errmsg("support for `code` BSON type is "
                                       "not implemented"),
                                errhint("Code: %s", BSON_ITER_CODE(&i))));
                break;
            ...
            case BSON_TYPE_INT32:
                appendStringInfo(output, "%d", BSON_ITER_INT32(&i));
                break;
            case BSON_TYPE_INT64:
                appendStringInfo(output, "%ld", (uint64_t)BSON_ITER_INT64(&i));
                break;
            ...
            case BSON_TYPE_DOCUMENT:
                ...
                uint32_t len;
                const uint8_t *data;
                bson_init_static(&tags, data, len);
                DumpJson(output, BSON_ITER_VALUE(&i), false);
                break;
            case BSON_TYPE_ARRAY:
                DumpJson(output, BSON_ITER_VALUE(&i), true);
                break;
            default:
                ereport(ERROR, (errcode(ERRCODE_FDW_INVALID_DATA_TYPE),
